...
from functions.cache_manager import cache
from functions.async_api_client import fetch_all_data_sync

def calcPositionsDetails(jsonPositionsDetailsInput: str) -> str:
    """
    Optimized version of calcPositionsDetails with caching and concurrent API calls
    """
    start_time = time.time()
    print(f"Starting calcPositionsDetails at {time.time()}")
    ...
    cache_key_tickers = tuple(sorted(lstAllTickersRevised))
    cached_data = cache.get('historical_data', *cache_key_tickers, ttl_hours=4)
    
    if cached_data:
        print("Using cached historical data")
        dfPricesSplitAdj = pd.DataFrame(cached_data['prices_split_adj'])
        dfPricesFinalNonAdj = pd.DataFrame(cached_data['prices_final_non_adj'])
        dfAdjFactors = pd.DataFrame(cached_data['adj_factors'])
        dfDividendsNonAdj = pd.DataFrame(cached_data['dividends_non_adj'])
        ...
        # Reduce data extraction period for better performance
        ...
        cache.set('historical_data', cache_data, *cache_key_tickers, ttl_hours=4)
    ...
    print("Fetching external API data concurrently...")
    api_start = time.time()
    ...
    print(f"Total time so far: {time.time() - start_time:.2f}s")

    # Cache external data for short-term reuse
    cache.set('quotes_data', quotes_data, *lstAllTickersRevised, ttl_hours=0.25)  # 15 minutes
    cache.set('options_data', options_data, *lstOptionTickers, ttl_hours=0.25)  # 15 minutes

    # Load earnings and dividends data with caching
    earnings_cache_key = ('earnings_data',)
    dividends_cache_key = ('dividends_data',)
    
    cached_earnings = cache.get('earnings_data', ttl_hours=24)
    cached_dividends = cache.get('dividends_data', ttl_hours=24)
    ...
    if cached_earnings and cached_dividends:
        ...
        # Cache the data
        cache.set('earnings_data', dfEarnings.to_dict(), ttl_hours=24)
        cache.set('dividends_data', dfDividends.to_dict(), ttl_hours=24)
